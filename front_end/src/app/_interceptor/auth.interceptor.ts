...
export class JwtInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    // add authorization header with jwt token if available console.log("Coming interceptor " + request.url.search(req) + " "+ request.url.search(req1))
    if (request.url.search(req) === -1 ) {
